...
class Runner(BaseRunner):
    ...
    def load_data(self, train_path=myconfigs.TRAIN_PATH, valid_path=myconfigs.VALID_PATH,
        ...
        if use_pca:
            ...
            self.validset.data =pd.DataFrame(index=alldata.index[idx1:idx1+idx2],columns=range(tempdata.shape[1]),data=tempdata[idx1:idx1+idx2])
            ...
            self.validset.data['SalePrice'] =y[idx1:idx1+idx2]
    ...
    def save_normed_dummy_data(self):
        self.trainset.data.to_csv("../data/normed_dummy_train.csv")
        self.validset.data.to_csv("../data/normed_dummy_valid.csv")
        self.testset.data.to_csv("../data/normed_dummy_test.csv")
